...
import { getProductsByHandles } from '@/src/services/shopify';
...
function WishlistContent() {
        ...
        async function fetchShared() {
            ...
            try {
                if (sharedUserId) {
                    ...
                    const profileRes = await fetch(`/api/profile?userId=${sharedUserId}`);
                    ...
                    // Fetch wishlist via API
                    const wishlistRes = await fetch(`/api/wishlist?userId=${sharedUserId}`);
    ...
    const handleLaVaca = async (product) => {
        // TODO: Implement vaca with new API
                        ...
                        <div className="flex-1 space-y-4 text-center md:text-left">
                            ...
                            >
                                <span className="text-blue-600 dark:text-blue-500 font-black uppercase tracking-[0.4em] text-[10px]">
                                    {isSharedView ? `Perfil de Usuario` : 'Tu Selección Personal'}
                                </span>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 pt-4">
                                {isSharedView ? (
                                    <>
                                        <Link
                                            href={`/chat?user=${ownerProfile?.id}`}
                ...
                <AnimatePresence mode="wait">
                    {loading ? (
                        ...
                        >
                            ...
                            <Link href="/" className="inline-block px-14 py-6 bg-black dark:bg-white text-white dark:text-black font-black rounded-full uppercase tracking-widest text-xs hover:scale-110 active:scale-95 transition-all shadow-xl">
                                Explorar Colección
                            </Link>
                        </motion.div>
                    )}
