...
import { getCollectionProducts } from '@/src/services/shopify';
...
export default function ProfilePage() {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-white dark:bg-black">
                <Header />
                <main className="max-w-md mx-auto px-6 pt-48 pb-24">
                    <div className="animate-pulse space-y-8">
                        <div className="w-24 h-24 bg-zinc-200 dark:bg-zinc-800 rounded-full mx-auto" />
                        <div className="h-8 bg-zinc-200 dark:bg-zinc-800 rounded-lg w-3/4 mx-auto" />
                        <div className="h-4 bg-zinc-100 dark:bg-zinc-900 rounded w-1/2 mx-auto" />
                        <div className="space-y-3 pt-8">
                            <div className="h-12 bg-zinc-100 dark:bg-zinc-900 rounded-2xl" />
                            <div className="h-12 bg-zinc-100 dark:bg-zinc-900 rounded-2xl" />
                        </div>
                    </div>
                </main>
    ...
    useEffect(() => {
        ...
        async function fetchUsers() {
            try {
                const res = await fetch(`/api/profile${userProfile?.id ? `?excludeId=${userProfile.id}` : ''}`);
                if (res.ok) {
                    const { profiles } = await res.json();
                    if (profiles) setOtherUsers(profiles);
                }
            } catch (err) {
    ...
    const handleAvatarUpload = async (e) => {
        ...
        try {
            ...
            // 1. Upload directly to S3 via our client helper
            const publicUrl = await uploadToS3(file, 'avatars');

            // 2. Update Profile via API (handled by updateProfile context function)
            await updateProfile({ avatar_url: publicUrl });

            alert('¡Listo! Foto actualizada.');
        } catch (error) {
            console.error('Error uploading avatar:', error);
            alert('Vaya, no pudimos subir la foto. Revisa la consola para más detalles.');
        } finally {
            setUploading(false);
        }
    };
    ...
    const handleSearch = async (e) => {
        ...
        if (query.trim().length > 1) {
            try {
                const res = await fetch(`/api/profile?query=${encodeURIComponent(query)}${userProfile?.id ? `&excludeId=${userProfile.id}` : ''}`);
                if (res.ok) {
                    const { profiles } = await res.json();
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div key="form" className="space-y-4">
                            ...
                            <button
                                onClick={handleAuth}
                                disabled={uploading}
                                className="w-full py-6 bg-black dark:bg-white text-white dark:text-black font-black rounded-3xl uppercase tracking-widest text-sm hover:scale-[1.02] active:scale-95 transition-all shadow-xl mt-4 disabled:opacity-50 flex items-center justify-center gap-3"
                            >
                                {uploading && <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />}
                            ...
                            {!forgotPasswordMode && isLoginMode && (
                                <div className="space-y-4 pt-4">
                                    ...
                                    <button
                                        ...
                                        disabled={uploading}
                                        className="w-full py-5 bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-3xl flex items-center justify-center gap-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all shadow-md group disabled:opacity-50"
                                    >
                                        ...
                                        <span className="text-sm font-bold text-black dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Continuar con Google</span>
                                    </button>
                                </div>
                            )}
                            ...
                            <div className="mt-12 flex flex-col items-center gap-2">
                                <div className="flex items-center gap-2 px-3 py-1 bg-green-500/10 border border-green-500/20 rounded-full">
                                    ...
                                    <p className="text-[10px] text-green-600 dark:text-green-500 font-black uppercase tracking-[0.2em]">Acceso Seguro con AWS + Redis</p>
                                </div>
                                <p className="text-zinc-400 text-[8px] uppercase font-bold tracking-widest mt-1 opacity-50">Producción Hardened v2.0</p>
                            </div>
                        </motion.div>
                    </AnimatePresence>
                </main>
                                ...
                                {userProfile.avatar_url ? (
                                    <img
                                        ...
                                        onError={(e) => {
                                            ...
                                            e.target.parentElement.innerHTML = userProfile.name?.[0]?.toUpperCase() || '👤';
                                        }}
                                    />
                                ) : (
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {orders?.length > 0 ? (
                                    ...
                                    <div className="py-20 text-center bg-zinc-900/30 rounded-[40px] border border-dashed border-white/5">
                                        ...
                                        <Link href="/" className="mt-4 inline-block text-blue-500 font-black uppercase text-[10px] hover:underline">Ir a la Tienda</Link>
                                    </div>
                                )}
                            </div>
                        </motion.div>
                                ...
                                {followRequests?.length > 0 && (
                                    <section className="space-y-6">
                                        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-blue-500 flex items-center gap-2">
                                            ...
                                            Solicitudes Pendientes ({followRequests?.length || 0})
                                        </h3>
                                                    ...
                                                    <div className="w-16 h-16 bg-zinc-200 dark:bg-zinc-800 rounded-2xl overflow-hidden flex-shrink-0 border border-black/10 dark:border-white/10 group-hover:border-white transition-all duration-300">
                                                        {user.avatar_url ? (
                                                            <img
                                                                ...
                                                                onError={(e) => {
                                                                    e.target.style.display = 'none';
                                                                    e.target.parentElement.innerHTML = user.name?.[0]?.toUpperCase() || '👤';
                                                                }}
                                                            />
                                                        ) : (
                            ...
                            {filteredWishlist?.length > 0 ? (
                                ...
                                <div className="space-y-16">
                                    <div className="text-center py-32 bg-zinc-50 dark:bg-zinc-900/30 rounded-[64px] border border-dashed border-black/5 dark:border-white/10">
                                        ...
                                        <Link href="/" className="inline-block px-12 py-5 bg-black dark:bg-white text-white dark:text-black font-black rounded-full uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition-all shadow-xl">Explorar Tienda</Link>
                                    </div>
                                </div>
                            )}
                        </motion.div>
