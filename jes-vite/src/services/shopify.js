...
/**
 * Shopify Fetch with Next.js Cache + Cloudflare Edge optimizations
 * 
 * Cache strategy:
 * - Products/Collections: 60s revalidate (stale-while-revalidate)
 * - Inventory/Cart: No cache (real-time)
 * - Single products: 30s revalidate
 * 
...
async function shopifyFetch({ query, variables, cache = {} }) {
  ...
  // Default cache: 60 seconds for reads, configurable per query
  const {
    revalidate = 60,  // seconds before revalidation
    tags = ['shopify'],  // cache tags for on-demand revalidation
    noCache = false  // force no-cache for mutations/inventory
  } = cache;
  ...
  // Add Next.js cache configuration (works with Cloudflare)
  if (!noCache && typeof window === 'undefined') {
    // Server-side: use Next.js fetch cache
    fetchOptions.next = {
